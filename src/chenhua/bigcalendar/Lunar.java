...
class Lunar {
	...
	public Lunar() {
		...
		setGregorian(now.get(Calendar.YEAR), now.get(Calendar.MONTH) + 1, now.get(Calendar.DAY_OF_MONTH));
	}
	...
	public String[] getYearTable() {
		...
		table[1] = getTextLine(27, "农历年历：" + (lunarYear + 1) + " ("
				+ stemNames[(lunarYear + 1 - 1) % 10]
				+ branchNames[(lunarYear + 1 - 1) % 12] + " - "
				+ animalNames[(lunarYear + 1 - 1) % 12] + "年)");
		int ln = 2;
		String blank = "                                         " + "  "
				+ "                                         ";
		String[] mLeft = null;
	...
	public String getDateString() {
		...
		if (gm.length() == 1)
			gm = ' ' + gm;
		String cm = String.valueOf(Math.abs(lunarMonth));
		if (cm.length() == 1)
			cm = ' ' + cm;
		String gd = String.valueOf(gregorianDate);
		if (gd.length() == 1)
			gd = ' ' + gd;
		String cd = String.valueOf(lunarDate);
		if (cd.length() == 1)
			cd = ' ' + cd;
		...
		} else if (lunarDate == 1 && lunarMonth > 0) {
			str = " " + lunarMonthNames[lunarMonth - 1] + "月";
		} else if (lunarDate == 1 && lunarMonth < 0) {
			str = "*" + lunarMonthNames[-lunarMonth - 1] + "月";
		} else {
			str = gd + '/' + cd;
